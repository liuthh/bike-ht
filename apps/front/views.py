...
def searchShop():
    ...
    if content:
        content = '%' + content + '%'
        if sort == 1:
            shops = GoodsModel.query.order_by(GoodsModel.create_time.desc()).filter(
                GoodsModel.title.like(content)).slice(start, end).all()
            if shops:
            ...
            shops = GoodsModel.query.order_by(GoodsModel.price.desc()).filter(GoodsModel.title.like(content)).slice(
                start, end).all()
            if shops:
                count = GoodsModel.query.filter(GoodsModel.title.like(content)).count()
                page = math.ceil(count / 8.0)
        ...
        if goods:
            ...
            if cart1:
                if CartModel.query.filter_by(user_id=user.id, goods_id=goods.id).first():
                    sql = "UPDATE cart_goods_middle SET number =number+{} WHERE cart_id ={} and goods_id={}".format(
                        number, cart1.id, goods.id)
    ...
    if form.validate():
        ...
        if types == 1:
            ...
            if count <= 1:
                ...
                sql = "UPDATE cart_goods_middle SET number =number-1 WHERE cart_id ={} and goods_id={}".format(cart.id,
                                                                                                               goods_id)  # 查询购物车的这个商品
                res = cur.execute(sql)  # 执行sql语句
                dbMy.commit()
...
@bp.route('/genarateOrderAll/', methods=['POST'])  # 购物车结算商品
@RequestLogin
def genarateOrderAll():
    '''
     :param: 数量（number），商品总价（price）,商品编号（good_id）,收货地址编号（address_id）
        :return:   code  305   必须登录
                   code  412   表单验证有错误
